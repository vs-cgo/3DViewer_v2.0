...
static void SaveGif(GifByteType *OutputBuffer, int Width, int Height,
  ...
  GifQprintf("\n%s: Image 1 at (%d, %d) [%dx%d]:     ", PROGRAM_NAME,
             GifFile->Image.Left, GifFile->Image.Top, GifFile->Image.Width,
...
static void RGB2GIF(bool OneFileFlag, int NumFiles, char *FileName,
  ...
  if (NumFiles == 1) {
    LoadRGB(FileName, OneFileFlag, &RedBuffer, &GreenBuffer, &BlueBuffer, Width,
            Height);
  } else {
    LoadRGB(NULL, OneFileFlag, &RedBuffer, &GreenBuffer, &BlueBuffer, Width,
            Height);
  }
  ...
  if (GifQuantizeBuffer(Width, Height, &ColorMapSize, RedBuffer, GreenBuffer,
                        BlueBuffer, OutputBuffer,
  ...
  do {
    ...
    switch (RecordType) {
      case IMAGE_DESC_RECORD_TYPE:
        ...
        GifQprintf("\n%s: Image %d at (%d, %d) [%dx%d]:     ", PROGRAM_NAME,
        ...
        if (GifFile->Image.Interlace) {
          ...
          for (Count = i = 0; i < 4; i++) {
            for (j = Row + InterlacedOffset[i]; j < Row + Height;
              ...
              if (DGifGetLine(GifFile, &ScreenBuffer[j][Col], Width) ==
                  GIF_ERROR) {
                PrintGifError(GifFile->Error);
                exit(EXIT_FAILURE);
              }
          ...
          for (i = 0; i < Height; i++) {
            GifQprintf("\b\b\b\b%-4d", i);
            if (DGifGetLine(GifFile, &ScreenBuffer[Row++][Col], Width) ==
                GIF_ERROR) {
              PrintGifError(GifFile->Error);
              exit(EXIT_FAILURE);
            }
